...
import edu.greenblitz.robotName.utils.systemCheck.SystemCheckBatteryConstants;
import edu.greenblitz.robotName.utils.systemCheck.SystemCheckConstants;
...
public class SystemCheck extends GBSubsystem {
    ...
    private void initBatteryWidget() {
        ...
        batteryDataList.addBoolean("battery overall good?",
                () -> getInnerBatteryResistance() < edu.greenblitz.robotName.utils.systemCheck.SystemCheckBatteryConstants.MAX_INNER_BATTERY_RESISTANCE &&
    ...
    public void initPingableDashboard() {
        ShuffleboardLayout pingableDataList = tab.getLayout("pingable", BuiltInLayouts.kList)
                .withPosition(6, 0).withSize(edu.greenblitz.robotName.utils.systemCheck.PingableManager.getInstance().getPingableList().toArray().length / 4, edu.greenblitz.robotName.utils.systemCheck.PingableManager.getInstance().getPingableList().toArray().length + 1)
                .withProperties(Map.of("Label position", "TOP",
                        "Number of columns", edu.greenblitz.robotName.utils.systemCheck.PingableManager.getInstance().getPingableList().toArray().length / 8 + 1,
                        "Number of rows", edu.greenblitz.robotName.utils.systemCheck.PingableManager.getInstance().getPingableList().toArray().length));
    ...
    public void initCheckCommands() {
        add(
                new CheckWristToAngle(
                        Rotation2d.fromDegrees(edu.greenblitz.robotName.utils.systemCheck.SystemCheckConstants.CHECK_WRIST_ANGLE)
                ),
                "wrist to angle"
        );
        ...
        add(
                new CheckElbowToAngle(
                        Rotation2d.fromDegrees(edu.greenblitz.robotName.utils.systemCheck.SystemCheckConstants.CHECK_ELBOW_ANGLE)
                ),
                "elbow to angle"
        );
        ...
        add(
                new CheckPivotToAngle(
                        Rotation2d.fromDegrees(edu.greenblitz.robotName.utils.systemCheck.SystemCheckConstants.CHECK_PIVOT_ANGLE)
                ),
                "pivot to angle"
        );
    ...
    public void add(SystemCheckCommand command, String checkName) {
        addToSeqCommand(command.raceWith(new WaitCommand(edu.greenblitz.robotName.utils.systemCheck.SystemCheckConstants.DEFAULT_COMMAND_TIME)));
        Debouncer debouncer = new Debouncer(edu.greenblitz.robotName.utils.systemCheck.SystemCheckConstants.DEFAULT_DEBOUNCING_TIME, Debouncer.DebounceType.kBoth);
